...
class EnzymeAssay(FlaskForm):
    ...
    fit = RadioField('Curve fit', choices=[('hyperbolic', 'Hyperbolic'), ('linear', 'Linear (K<sub>M</sub> >> [S])')], default='hyperbolic', validators=[DataRequired()])
...
def home():
    ...
    if form.validate():
        ...
        calc = kinetics_calc(assay_df, fit, **enz_dict)
        calc.plot_mm(title)
